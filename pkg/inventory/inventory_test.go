...
func Test_host_get_not_found(t *testing.T) {
	...
	_, err := Get_hostdata("no-such-host")
	if (err == nil) {
...
func Test_host_set_state(t *testing.T) {
	...
	err := Set_host_state("h1", openapi.CSTATE_FAILED)
	if (err != nil) {
...
func Test_host_set_state_not_found(t *testing.T) {
	reset()
	err := Set_host_state("no-such-host", openapi.CSTATE_ACTIVE)
	if (err == nil) {
...
func Test_vm_add_and_get(t *testing.T) {
	...
	if (vm.Uuid != "vm1") {
		t.Errorf("Uuid: got %q", vm.Uuid)
	}
	if (vm.Name != "testvm") {
		t.Errorf("Name: got %q", vm.Name)
	}
	if (vm.Host != "h1") {
		t.Errorf("Host: got %q", vm.Host)
	}
	if (vm.Runstate != openapi.RUNSTATE_RUNNING) {
		t.Errorf("Runstate: got %d", vm.Runstate)
	}
...
func Test_vm_state_update_runstate(t *testing.T) {
	...
	err := Update_vm_state(vm_event("vm1", "h1", openapi.RUNSTATE_PAUSED, 200))
	if (err != nil) {
...
func Test_vm_state_update_deleted(t *testing.T) {
	...
	err := Update_vm_state(vm_event("vm1", "h1", openapi.RUNSTATE_DELETED, 200))
	if (err != nil) {
...
func Test_vm_state_update_migration(t *testing.T) {
	...
	err := Update_vm_state(vm_event("vm1", "h2", openapi.RUNSTATE_RUNNING, 200))
	if (err != nil) {
...
func Test_vm_state_update_unknown_vm(t *testing.T) {
	reset()
	err := Update_vm_state(vm_event("no-such-vm", "h1", openapi.RUNSTATE_RUNNING, 100))
	if (err == nil) {
...
func Test_search_vms_by_custom_field(t *testing.T) {
	...
	if (err != nil) {
		t.Fatalf("Update_vm vm1: %v", err)
	}
	err = Update_vm(vm_nomatch)
	if (err != nil) {
