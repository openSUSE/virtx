...
func host_get(host *openapi.Host) {
	if (virtx.stat_cpu) {
		...
		fmt.Fprintf(virtx.w, "NAME\tOS\tVERSION\tCPU\tVENDOR\tMODEL\tNODES\tSOCKS\tCORES\tTH\tMEM_AVL_VM\tHPG_AVL_VM\tTSC_FREQ\tFWVER\tFWDATE\tCSTATE\n")
		fmt.Fprintf(virtx.w, "%s\t%s\t%s\t%s\t%s\t%s\t%5d\t%5d\t%5d\t%2d\t%7d MiB\t%7d MiB\t%d\t%s\t%s\t%s\n",
			...
			host.Def.Tscfreq, host.Def.Sysinfo.Version, host.Def.Sysinfo.Date, host.Cstate,
		)
	}
}
