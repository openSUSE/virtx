...
func host_list(list *openapi.HostList) {
	...
	fmt.Fprintf(virtx.w, "UUID\tNAME\tOS\tVERSION\tCPU\tVENDOR\tMODEL\tTHREADS\t MEM_AVL_VM\t HPG_AVL_VM\tSTATE\n")

	for _, item := range (list.Items) {
		fmt.Fprintf(virtx.w, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%7d\t%7d MiB\t%7d MiB\t%s\n",
